...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="MultipleStringConnector" as="xs:string">
	...
	<xsl:variable name="DisplayNote" as="xs:string">
		<xsl:value-of select="'Any prescribed medications not noted above should be taken as prescribed. If you have a question or concern regarding any of your other medications please contact the prescribing provider.'"/>
	</xsl:variable>

	<xsl:variable name="UnchangedMedsDisplayNote" as="xs:string">
		<xsl:value-of select="'Any prescribed medications should be taken as prescribed. If you have a question or concern regarding any of your other medications please contact the prescribing provider.'"/>
	</xsl:variable>
	...
	<xsl:variable name="ShowHowMuchColNonStop" as="xs:boolean">
		<xsl:choose>
			<xsl:when test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP') and @detail-line-dose-type='SPECIAL_INSTRUCTIONS']">
				<xsl:value-of select="true()"/> <!-- At least a SPECIAL_INSTRUCTIONS med exists -->
			</xsl:when>
			<xsl:when test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]/n:report-order-detail[@detail-field-meaning='FREETEXT_DOSE']">
				<xsl:value-of select="true()"/> <!-- At least a med with a free text dose exists -->
			</xsl:when>
			<xsl:when test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]/n:report-order-detail[@detail-field-meaning='VOLUME_DOSE']">
				<xsl:value-of select="true()"/> <!-- At least a med with a volume dose exists -->
			</xsl:when>
			<xsl:when test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]/n:report-order-detail[@detail-field-meaning='STRENGTH_DOSE']">
				<xsl:value-of select="true()"/> <!-- At least a med with a strength dose exists -->
			</xsl:when>
			<xsl:when test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]/n:report-order-detail[@detail-field-meaning='ROUTE_OF_ADMINISTRATION']">
	...
	<xsl:variable name="ShowWhyColNonStop" as="xs:boolean">
		<xsl:choose>
			<xsl:when test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]/n:diagnoses/n:diagnosis">
				<xsl:value-of select="true()"/> <!-- At least a med has one or more associated diagnoses -->
			</xsl:when>
			<xsl:when test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]/@order-indication">
	...
	<!-- function Name : cdocfx:getPRNDescription -->
	<!-- param 1 Name  : PRNIndicator -->
	<!-- param 2 Name  : PRNInstructions -->
	<!-- param 3 Name  : PRNReason -->
	<!-- description   : this fuction accepts PRNIndicator,PRNInstructions and PRNReason as parameters and returns the values of PRNs based on the value of PRNIndicator. -->
	<xsl:function name="cdocfx:getPRNDescription" as="xs:string">
		<xsl:param name="PRNIndicator"/>
		<xsl:param name="PRNInstructions"/>
		<xsl:param name="PRNReason"/>
		<xsl:choose>
	...
	<xsl:template match="/">
		...
		<xsl:variable name="UniquePharmacies">
			<xsl:for-each-group select="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]/n:routing-information/n:pharmacy" group-by="./n:name/text()">
		...
		<xsl:variable name="ShowNotStopMedsTable" as="xs:boolean" select="($ShowUnchangedMeds and n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')])
			or n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP') and not(@report-order-type eq 'CONTINUE') and not(@report-order-type eq'CONTACT_PHYSICIAN')]"/>
		...
		<xsl:if test="n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'STOP')]">
			<xsl:if test="$ShowNotStopMedsTable">
				<table style="table-layout:fixed; border:1px solid #000; border-collapse:collapse;" dd:zebrastripecolor="#F4F4F4" dd:zebraheadercolor="#F4F4F4" dd:whitespacecolseparator="true"> <!-- Separate columns by a space in plain text conversion -->
					<thead style="display:table-header-group;"> <!-- This thead will repeat table heading on each page when printing on paper such as Draft Print -->
						<tr>
							...
							<xsl:if test="$ShowHowMuchColNonStop">
			...
			<!-- List of Pharmacies displayed after a Medication Reconciliation NOT STOP table. Check if table present before displaying -->
			<xsl:if test="exists($UniquePharmacies/pharmacy) and $ShowNotStopMedsTable">
				<div><b>Pharmacy Information </b></div>
				<xsl:for-each select="$UniquePharmacies/pharmacy">
					<div style="text-indent: -1em; margin-left: 0em; padding-left: 1em;">
				...
				<thead style="display:table-header-group;"> <!-- This thead will repeat table heading on each page when printing on paper such as Draft Print -->
					<tr>
						...
						<xsl:if test="$ShowHowMuchColStopTaking">
							<th style="max-width:10%; vertical-align:top; text-align:center; padding:0 5px; border-bottom: 1px solid #000; border-left:1px solid #000; border-right:1px solid #000; font-weight:bold;"><xsl:value-of select="$HowMuch"/></th>
						</xsl:if>
						<th style="max-width:10%; vertical-align:top; text-align:center; padding:0 5px; border-bottom: 1px solid #000; border-left:1px solid #000; border-right:1px solid #000; font-weight:bold;"><xsl:value-of select="$When"/></th>
						
						<xsl:if test="$ShowWhyColStopTaking">
							<th style="max-width:10%; vertical-align:top; text-align:center; padding:0 5px; border-bottom: 1px solid #000; border-left:1px solid #000; border-right:1px solid #000; font-weight:bold;"><xsl:value-of select="$Why"/></th>
						</xsl:if>
						
						<th style="max-width:10%;vertical-align:top; text-align:center; padding:0 5px; border-bottom: 1px solid #000; border-left:1px solid #000; border-right:1px solid #000; font-weight:bold;"><xsl:value-of select="$Comments"/></th>
					</tr>
				</thead>
				<tbody>
					<!-- Stop Taking -->
		...
		<!-- Display a note based on the medications present-->
		<xsl:choose>
			<xsl:when test="not($ShowUnchangedMeds) and n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[@report-order-type eq 'FILL' or @report-order-type eq 'CONTINUE_WITH_CHANGES' or @report-order-type eq 'STOP']">
				<br/>
				<div><xsl:value-of select="$DisplayNote"/></div>
			</xsl:when>

			<xsl:when test="not($ShowUnchangedMeds) and n:report/n:clinical-data/n:reconciliation-report-data/n:reconciliation-report-order[not(@report-order-type eq 'FILL') and not(@report-order-type eq 'CONTINUE_WITH_CHANGES') and not(@report-order-type eq 'STOP')]">
				<div><xsl:value-of select="$UnchangedMedsDisplayNote"/></div>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="DisplayMed">
		...
		<!--  dd:refreshid values for the freetext areas should be unique  -->
		<tr class="ddemrcontentitem ddremovable" style="border-top: 1px solid #000; page-break-inside: avoid;" dd:btnfloatingstyle="top-right">
			...
			<xsl:if test="not($IsSpecialInstruction)">
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:variable name="PRNDescription" as="xs:string" select="cdocfx:getPRNDescription($PRNIndicator,$PRNInstructions,$PRNReason)"/>
						<xsl:choose>
			...
			<xsl:if test="$ShowWhyCol">
				<td style="max-width:10%; vertical-align:top; padding:0 5px; border-left:1px solid #000; border-right:1px solid #000;">
					<!-- Diagnoses -->
					<xsl:if test="$Med/n:diagnoses/n:diagnosis">
						<xsl:for-each select="$Med/n:diagnoses/n:diagnosis">
							<div class="ddgrouper ddremovable" dd:btnfloatingstyle="top-right" style="margin-left:0em; text-indent:-1em; padding-left:1em;" ><xsl:value-of select="@annotated-display"/></div>
						</xsl:for-each>
					</xsl:if>
					...
					<xsl:if test="$OrderIndication!='' and not($Med/n:diagnoses/n:diagnosis)">
						<div class="ddgrouper ddremovable" dd:btnfloatingstyle="top-right" style="margin-left:0em; text-indent:-1em; padding-left: 1em;"><xsl:value-of select="$OrderIndication"/></div>
					</xsl:if>
			...
			<xsl:variable name="commaSeperator">
				<xsl:if test="$SpecialInstruct!=''">
					<xsl:value-of select="', '"/>
				</xsl:if>
			</xsl:variable>

			<!-- Associating special instructions with PRN instruction in case of free text dose meds. -->
			<xsl:variable name="PRNDescription" as="xs:string" select="cdocfx:getPRNDescription($PRNIndicator,$PRNInstructions,$PRNReason)"/>
			...
			<xsl:variable name="WordWrapBreakInstructions" as="xs:boolean">
				<xsl:choose>
					<xsl:when test="$IsSpecialInstruction">
						<xsl:call-template name="containsLongString">
							<xsl:with-param name="count" select="0"></xsl:with-param>
							<xsl:with-param name="StringToEvaluate" select="java-string:format($MultipleStringConnector,(cdocfx:space-after-slash($SpecialInstruct),cdocfx:space-after-slash($PRNDescription),cdocfx:space-after-slash($Med/n:patient-instructions/text())))"></xsl:with-param>
							<xsl:with-param name="CharacterLimit" select="$CharacterLimit"></xsl:with-param>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="containsLongString">
							<xsl:with-param name="count" select="0"></xsl:with-param>
							<xsl:with-param name="StringToEvaluate" select="java-string:format($Connector,(cdocfx:space-after-slash($SpecialInstruct),cdocfx:space-after-slash($Med/n:patient-instructions/text())))"></xsl:with-param>
							<xsl:with-param name="CharacterLimit" select="$CharacterLimit"></xsl:with-param>
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>
			...
			<xsl:variable name="SpecialInstructionsWithPRNInstruction" as="xs:string">
				<xsl:choose>
					<xsl:when test="$Med/@detail-line-dose-type='SPECIAL_INSTRUCTIONS' and $PRNDescription!=''">
						<xsl:value-of select="concat($SpecialInstruct,$commaSeperator,cdocfx:upper-first-char(java-string:format($PRNConnector,($PRNDescription))))"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="$SpecialInstruct"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			...
			<td>
				...
				<!-- Output SPECIAL_INSTRUCTIONS and PRN incase of free text dose meds -->
				<xsl:if test="$SpecialInstructionsWithPRNInstruction!=''">
					<div class="ddgrouper ddremovable" dd:btnfloatingstyle="top-right"><xsl:value-of disable-output-escaping="yes" select="doc:convertPlainTextToHtml(cdocfx:space-after-slash($SpecialInstructionsWithPRNInstruction))"/></div>
				</xsl:if>
				...
				<xsl:if test="($Med/n:routing-information/n:pharmacy or $Med/n:routing-information/@rx-printed-indicator='1') and not($Med/@report-order-type eq 'STOP')" >
				...
				<div class="ddfreetext" dd:refreshid="instructions"></div>
			</td>

			<xsl:if test="$ShowNextLast">
				<td style="vertical-align:top; padding:0 5px; border-left:1px solid #000; border-right:1px solid #000;">
					<div class="ddfreetext" dd:refreshid="nextDose"/>
				</td>
			</xsl:if>
		</tr>
