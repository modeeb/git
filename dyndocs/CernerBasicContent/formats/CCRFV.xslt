		...
		<span class="ddemrcontentitem ddremovable">
			...
			<xsl:choose>
				<xsl:when test="$lastDx = true()">
					<xsl:choose>
						<xsl:when test="$displayConfirmation">
							<xsl:value-of select="concat(cdocfx:getDxDisplay($dx),cdocfx:getDiagnosesConfirmationDisplay($dx))"/>
						</xsl:when>
						<xsl:otherwise>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="$displayConfirmation">
							<xsl:value-of select="java-string:format($Separator, concat(cdocfx:getDxDisplay($dx),cdocfx:getDiagnosesConfirmationDisplay($dx)))" disable-output-escaping="yes"/>
						</xsl:when>
						<xsl:otherwise>
