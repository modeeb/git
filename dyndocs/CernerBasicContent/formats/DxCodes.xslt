...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- Parameters: -->
	<!--	dx - diagnosis object -->
	<xsl:function name="cdocfx:getDiagnosisDisplay" as="xs:string">
		<xsl:param name="dx"/>
		<xsl:choose>
	...
	<xsl:template name="tempClinicalDiagnosis" >
		...
		<xsl:choose>
			<!-- Display Confirmation status of the diagnosis except for 'CONFIRMED' diagnoses -->
			<xsl:when test="$displayConfirmation">
				<xsl:value-of select="concat(cdocfx:getNomenclatureDescByID($dx/n:diagnosis-name/n:nomenclature),cdocfx:getDiagnosesConfirmationDisplay($dx))"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="/">
		...
		<xsl:variable name="dxSorted">
			<xsl:perform-sort select="n:report/n:clinical-data/n:diagnosis-data/n:clinical-diagnosis">
				<xsl:sort select="fn:upper-case(cdocfx:getDiagnosisDisplay(.))" order="ascending"/>
			</xsl:perform-sort>
		</xsl:variable>
