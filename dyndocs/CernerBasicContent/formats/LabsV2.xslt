...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<!-- Given a coded measurement value for labs EMR, this function will display the nomenclature short description for the lab value instead of the nomenclature long description. -->
	<!-- Parameters: -->
	<!-- 	value - the measurement value node -->
	<xsl:function name="cdocfx:getMeasurementValueDisplayLabs">
		<xsl:param name="measurement" as="element()?"/>

		<xsl:choose>
			<xsl:when test="$measurement/n:value/n:coded">
				<xsl:for-each select="$measurement/n:value/n:coded/n:code-value">
					<xsl:if test="n:nomenclature"><xsl:value-of select="cdocfx:getNomenclatureShortDescByID(n:nomenclature)" /></xsl:if>
					<xsl:if test="position() != last()"><xsl:value-of select="$multiValueSeparator" /></xsl:if>
				</xsl:for-each>
				<xsl:if test="$measurement/n:value/n:coded/@other-response">
					<xsl:if test="$measurement/n:value/n:coded/n:code-value"><xsl:value-of select="$multiValueSeparator" /></xsl:if>
					<xsl:value-of select="$measurement/n:value/n:coded/@other-response" />
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="cdocfx:getMeasurementValue($measurement,$DATE_SEQUENCE,$DATE_ONLY_SEQUENCE,$measValueUnit)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:function>
			...
			<xsl:if test="($isTagged or xs:dateTime($updateDtTmUTC) &lt;= xs:dateTime($pendingDateTimeThreshold)) and cdocfx:isStatusCodeAuthenticated($eventStatusMean)">
				...
				<measurement>					
					<xsl:variable name="normalcyMean" as="xs:string">
						<xsl:value-of select="cdocfx:getMeasurementInterpretationLabs(.)"/>
					</xsl:variable>
					
					<xsl:variable name="measurementValue" as="xs:string">
						<xsl:value-of select="cdocfx:getMeasurementValueDisplayLabs(.)"/>
					</xsl:variable>
					...
					<xsl:attribute name="interpretation" select="$normalcyMean"/>
					<xsl:attribute name="column-two" select="$measurementValue"/>
					<xsl:attribute name="column-three" select="$collected-dt-tm"/>
					<xsl:if test="$showResultComments and $measurementComment">
						<xsl:attribute name="comment" select="$measurementComment"/>
					</xsl:if>
	...
	<xsl:template match="/">
		...
		<xsl:variable name="commentColHeading" as="xs:string" select="if(($showMostRecentResults and $sortedPatientLabMeasurements/measurement/@comment) or (not($showMostRecentResults) and $sortedLabMeasurements/measurement/@comment)) then $ColumnHeadingFour else ''" />
		
		<!-- Parameters Explanation  -->
		<!-- colHeadingOne, colHeadingTwo, colHeadingThree: - Tells about the column heading  -->
		<!-- spanningRowTwoLabel :- This is the content which has to be added as a row   -->
		<!-- i.e., this row(spanningRowTwoLabel) is associated with the previous row -->
		<xsl:call-template name="tempMeasurementsTable">
			...
			<xsl:with-param name="spanningRowTwoLabel" select="$commentColHeading"/>
		</xsl:call-template>
	</xsl:template>
</xsl:stylesheet>