...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="xsl xs fn n cdocfx java-string doc">
	...
	<xsl:template match="n:report/n:clinical-data/n:radiology-data/n:radiology-document" mode="preprocessing">
		...
		<radDocument>
			...
			<xsl:for-each select="n:radiology-exam">
				...
				<xsl:if test="$orderStatusMean = 'COMPLETED' and ((xs:dateTime($updateDtTmUTC) &gt;= xs:dateTime($pendingDateTimeThreshold)) or ($showAllCompletedResults and (xs:dateTime($updateDtTmUTC) &lt;= $currentDateTime)))">
