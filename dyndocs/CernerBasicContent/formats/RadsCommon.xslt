...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="pendingDateTimeThreshold" as="xs:dateTime" select="$currentDateTime - xs:dayTimeDuration('PT24H')"/>
	...
	<xsl:template name="sortRadiologyExams">
		...
		<xsl:for-each select="$radExams">
			...
			<!-- Format the radiology result by honoring the showAllCompletedResults flag and if its status satisfies the following constraints. -->
			<xsl:if test="$orderStatusMean = 'COMPLETED' and cdocfx:isStatusCodeAuthenticated($examStatusMean) and ((xs:dateTime($updateDtTmUTC) &gt;= xs:dateTime($pendingDateTimeThreshold)) or ($showAllCompletedResults and (xs:dateTime($updateDtTmUTC) &lt;= $currentDateTime)))">
