...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable as="xs:string" name="FamilyMemberDisp" select="'%s %s %s'"/>
	...
	<xsl:variable as="xs:string" name="DeceasedRelationDisplay" select="'Relationship: %s'"/>
	<xsl:variable as="xs:string" name="DeceasedRelationNameDisplay" select="', Name: %s'"/>
	<xsl:variable as="xs:string" name="DeceasedAgeDisplay" select="', Age: %s'"/>
	<xsl:variable as="xs:string" name="DeceasedCauseofDeathDisplay" select="', Cause: %s'"/>
	<xsl:variable as="xs:string" name="DeceasedFamilyLabel" select="'Deceased Family Member(s)'"/>
	<xsl:variable as="xs:string" name="HealthStatusFamilyLabel" select="'Health Status Family Member(s)'"/>
	<xsl:variable as="xs:string" name="ConditionCommentLabel" select="' (Comments: '"/>
	<xsl:variable as="xs:string" name="ConditionCommentSeparator" select="'; '"/>
	...
	<xsl:template name="FamilyMemberHistory">
		<xsl:for-each select="n:report/n:clinical-data/n:family-history-data/n:patient-condition/n:family-relationship">
			<xsl:if test="count(n:health-status) &gt; 0">
				...
				<div class="ddemrcontentitem ddremovable" style="margin-left: 0em; text-indent: -1em; padding-left: 1em;">
	...
	<!-- This template is used to check whether a family member is Deceased -->
	<xsl:template name="FamilyMemberDeceased">
		<xsl:for-each select="n:report/n:clinical-data/n:family-history-data/n:patient-condition/n:family-relationship/n:related-person">
			<xsl:if test="fn:exists(@is-deceased) and (@is-deceased eq 'true')">
				<div class="ddemrcontentitem ddremovable" style="margin-left: 0em; text-indent: -1em; padding-left: 1em;">
					<xsl:attribute name="dd:entityid"> <!-- This is the relationship-code-->
						<xsl:value-of select="n:related-person/@relationship-code"/>
					</xsl:attribute>
					...
					<!-- @relationship-code is required -->
					<xsl:variable name="Relationship" as="xs:string" select="cdocfx:getCodeDisplayByID(@relationship-code)"/>
					<xsl:value-of select="java-string:format($DeceasedRelationDisplay, $Relationship)"/>
					
					<xsl:if test="exists(n:name/@name-full) and not(n:name/@name-full eq '')">
						<xsl:variable name="RelationshipFullName" as="xs:string" select="n:name/@name-full"/>	
						<xsl:value-of select="java-string:format($DeceasedRelationNameDisplay, $RelationshipFullName)"/>
					</xsl:if>
					
					<xsl:if test="exists(@age-at-death) and not(@age-at-death eq '')">
						<xsl:variable name="AgeAtDeath" as="xs:string" select="@age-at-death"/>				
						<xsl:value-of select="java-string:format($DeceasedAgeDisplay, $AgeAtDeath)"/>
					</xsl:if>
					
					<xsl:if test="exists(@cause-of-death) and not(@cause-of-death eq '')">
						<xsl:variable name="CauseOfDeath" as="xs:string" select="@cause-of-death"/>
						<xsl:value-of select="java-string:format($DeceasedCauseofDeathDisplay, $CauseOfDeath)"/>		
					</xsl:if>
				</div>
			</xsl:if>		
		</xsl:for-each>
	...
	<xsl:function name="cdocfx:getFamilyMemberDisplay" as="xs:string">
		...
		<xsl:variable name="display" as="xs:string">
			...
			<xsl:variable name="ConditionComment">
				<xsl:choose>
					<xsl:when test="$ShowComments and exists($currentCondition/n:condition-comment/n:comment)">
						<xsl:call-template name="DisplayConditionComments">
							<xsl:with-param name="condition" select="$currentCondition"/>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="''"/>
			...
			<xsl:variable name="ConditionCommentStr" as="xs:string" select="$ConditionComment"/>
			
			<xsl:value-of select="concat($RelationshipTypeDisp, $OnsetAge, $ConditionCommentStr)"/>
		</xsl:variable>

		<xsl:value-of select="$display"/>
	</xsl:function>
	
	<xsl:template name="DisplayConditionComments">
		<xsl:param name="condition"/>
		
			<xsl:value-of select="$ConditionCommentLabel"/>
			<xsl:for-each select="$condition/n:condition-comment">
				<xsl:value-of select="n:comment"/>
				<xsl:if test="not(position() = last())">
					<xsl:value-of select="$ConditionCommentSeparator"/>
				</xsl:if>
				<xsl:if test="not(not(position() = last()))">
					<xsl:value-of select="')'"/>
				</xsl:if>
			</xsl:for-each>
	...
	<xsl:template match="/">
		...
		<xsl:choose>
			...
			<xsl:when test="count(n:report/n:clinical-data/n:family-history-data/n:patient-condition/n:family-relationship/n:condition) &gt; 0">
				...
				<xsl:if test="n:report/n:clinical-data/n:family-history-data/n:patient-condition/n:family-relationship">
					<div class="ddemrcontentitem ddremovable ddgrouper" style="margin-top: 10px;">
						<span style="font-weight: bold;text-decoration: underline;">
							<xsl:value-of select="$HealthStatusFamilyLabel"/>
						</span>
		...
		<!-- Display the family member(s) that are Deceased -->
		<xsl:choose>
			<xsl:when test="count(n:report/n:clinical-data/n:family-history-data/n:patient-condition/n:family-relationship/n:related-person[@is-deceased='true']) &gt; 0">
				<div class="ddemrcontentitem ddremovable ddgrouper" style="margin-top: 10px;">
					<span style="font-weight: bold;text-decoration: underline;">
						<xsl:value-of select="$DeceasedFamilyLabel"/>
					</span>
					<xsl:call-template name="FamilyMemberDeceased"/>
				</div>
			</xsl:when>
			<xsl:otherwise>
				<!-- Don't display anything -->
