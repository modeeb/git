...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="RelatedDiagnosisLabel" as="xs:string">
		<xsl:value-of select="' Related Diagnosis:  %s'"/>
	</xsl:variable>
	
	<xsl:variable name="CommentsLabel" as="xs:string">
		<xsl:value-of select="'Comments:  %s'"/>
	</xsl:variable>
	...
	<xsl:template match="n:report/n:clinical-data/n:procedure-data/n:clinical-procedure" mode="preprocessing">
		<procedure>
			...
			<xsl:variable name="Laterality">
				<xsl:choose>
					<xsl:when test="exists(n:laterality-code)">
						<xsl:value-of select="cdocfx:getCodeDisplayByID(n:laterality-code)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="''"/>
					</xsl:otherwise>
			...
			<!-- Related Diagnosis -->
			<xsl:variable name="RelatedDiagnosis">
				<xsl:choose>
					<xsl:when test="exists(n:procedure-related-diagnosis/n:related-diagnosis/n:freetext)">
						<xsl:for-each select="n:procedure-related-diagnosis/n:related-diagnosis">
					...
					<xsl:when test="exists(n:procedure-related-diagnosis/n:related-diagnosis/n:nomenclature)">
						<xsl:for-each select="n:procedure-related-diagnosis/n:related-diagnosis">
			...
			<xsl:variable name="Comments">
				<xsl:if test="fn:exists(n:procedure-comment/n:comment) and $ShowComments">
					<xsl:for-each select="n:procedure-comment">
						<xsl:choose>
			...
			<xsl:attribute name="display" select="cdocfx:getProcedureDisplay(.)"/>
			<xsl:attribute name="servicedate" select="$ServiceDate"/>
			<xsl:attribute name="laterality" select="$Laterality"/>
			<xsl:attribute name="relateddiagnosis" select="$RelatedDiagnosis"/>
			<xsl:attribute name="comments" select="$Comments"/>
			...
			<ul style="list-style-type: disc; margin: 0; padding-left: 15px">
				<xsl:for-each select="$SortableProcedures/procedure">
					...
					<li class="ddemrcontentitem ddremovable">
						...
						<!-- Put the procedures in a bulleted list -->
						<xsl:variable name="ProcedureDisplay" as="xs:string" select="@display"/>
						<xsl:value-of select="$ProcedureDisplay"/>
						
						<!-- Procedure laterality -->
						<xsl:if test="@laterality != ''">
							<xsl:variable name="LateralityDisplay" as="xs:string" select="@laterality"/>
							<xsl:value-of select="java-string:format($LateralityLabel,$LateralityDisplay)"/>
						</xsl:if>
						
						<!-- Procedure Date -->
						<xsl:if test="@servicedate != ''">
							<xsl:variable name="ServiceDateDisplay" as="xs:string" select="@servicedate"/>
							<xsl:value-of select="java-string:format($ServiceDtLabel,$ServiceDateDisplay)"/>
						</xsl:if>
						
						<!-- Procedure Date -->
						<xsl:if test="@relateddiagnosis != ''">
							<xsl:variable name="RelatedDiagnosisDisplay" as="xs:string" select="@relateddiagnosis"/>
							<xsl:value-of select="java-string:format($RelatedDiagnosisLabel,$RelatedDiagnosisDisplay)"/>
						</xsl:if>
						
						<!-- Procedure Comments -->
						<xsl:if test="@comments != ''">
							<div style="padding-left: 10px;">
								<xsl:variable name="CommentsDisplay" as="xs:string" select="@comments"/>
								<xsl:value-of disable-output-escaping="yes" select="java-string:format($CommentsLabel,$CommentsDisplay)"/>
							</div>
						</xsl:if>
					</li>
				</xsl:for-each>
