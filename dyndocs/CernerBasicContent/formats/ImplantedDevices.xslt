...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="MRPossiblyUnsafeAlert" select="'Notice: This patient has devices implanted this visit that may not be MRI compatible.'"/>
            ...
            <xsl:when test="$fuzzyDateElement/n:date">
                ...
                <xsl:choose>
                    <xsl:when test="$datePrecision = 'DATE_AND_TIME'">
                        <xsl:value-of select="xr-date-formatter:formatDate($fuzzyDate, $DATE_ONLY_SEQUENCE, $timezone, $current-locale)"/>
                    </xsl:when>
                    <xsl:when test="$datePrecision = 'DAY'">
                        <xsl:value-of select="xr-date-formatter:formatDate($fuzzyDate, $DATE_ONLY_SEQUENCE, $timezone, $current-locale)"/>
                    </xsl:when>
                    <xsl:when test="$datePrecision = 'MONTH_AND_YEAR'">
                        <xsl:value-of select="xr-date-formatter:formatDate($fuzzyDate, $MONTH_YEAR_SEQUENCE, $timezone, $current-locale)"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="xr-date-formatter:formatDate($fuzzyDate, $YEAR_SEQUENCE, $timezone, $current-locale)"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
                ...
                <xsl:value-of select="xr-date-formatter:formatDate($date, $DATE_ONLY_SEQUENCE, $timezone, $current-locale)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
