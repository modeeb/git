	...
	<xsl:variable name="DoCommentsExistNotGiven" as="xs:string">
		...
		<xsl:if test="$Immunizations/immunizationGiven">
			...
			<div class="ddgrouper ddremovable" dd:btnfloatingstyle="top-right">
				...
				<table style="table-layout: fixed;border-spacing:1.5em 0; border-collapse:collapse;border:1px solid #000; margin-left: 1em;" class="ddremovable" dd:btnfloatingstyle="top-right" dd:zebrastripecolor="#F4F4F4" dd:zebraheadercolor="#F4F4F4" dd:whitespacecolseparator="true">
					<thead style="display:table-header-group;"> <!-- This thead will repeat table heading on each page when printing on paper such as Draft Print -->
						...
						<tr>
							<th>
								<xsl:attribute name="style" select="if($providerFacing) then 'font-weight:normal; text-align:center; padding:0 5px; padding-left:6px; border-bottom: 1px solid #000 border-right:1px solid #000;;'
									else 'font-weight:bold; text-align:center;padding:0 5px; padding-left:6px; border-bottom: 1px solid #000;border-right:1px solid #000;'"/>
								<xsl:value-of select="$Vaccine"/>
							</th>
							<th> 
								<xsl:attribute name="style" select="if($providerFacing) then 'font-weight:normal; text-align:center; vertical-align:top; padding:0 5px; border-bottom: 1px solid #000; border-left:1px solid #000; padding-left:6px;border-right:1px solid #000;'
									else 'font-weight:bold; text-align:center; vertical-align:top; padding:0 5px; border-bottom: 1px solid #000; border-left:1px solid #000; padding-left:6px;border-right:1px solid #000;'"/>
		...
		<xsl:if test="$Immunizations/immunizationNotGiven">
			...
			<div class="ddgrouper ddremovable" dd:btnfloatingstyle="top-right">
				...
				<!-- Separate columns by a space in plain text conversion -->
				<table style="table-layout: fixed;border-spacing:1.5em 0; border-collapse:collapse;border:1px solid #000; margin-left: 1em;width = 100%" class="ddremovable" dd:btnfloatingstyle="top-right" dd:zebrastripecolor="#F4F4F4" dd:zebraheadercolor="#F4F4F4" dd:whitespacecolseparator="true">
					<thead style="display:table-header-group;"> <!-- This thead will repeat table heading on each page when printing on paper such as Draft Print -->
						<tr>
							<th>
			...
			<td colspan="2">
				<table dd:whitespacecolseparator="true" style="table-layout: fixed;width: 100%; border-collapse: collapse; border-spacing: 0;">
					...
					<tbody>
						...
						<xsl:if test="$ShowCommentsColumn='1' and $Immunization/comment">
									...
									<table style="table-layout: fixed;width:100%; border:0;" dd:whitespacecolseparator="true">
										...
										<tr>
											<td style="vertical-align:top;">
												<span>
													<xsl:attribute name="style" select="if($providerFacing) then 'white-space:nowrap;font-weight:normal; text-align:center;'
														else 'font-weight:bold; text-align:center;white-space:nowrap;'"/>
