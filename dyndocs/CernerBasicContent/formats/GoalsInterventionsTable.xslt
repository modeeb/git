...
<xsl:stylesheet version="2.0"
    ...
    <!--String defined for displaying a comma-space between multiple items-->
        ...
        <xsl:if test="@description != ''">
            ...
            <tr class="ddemrcontentitem ddremovable" style="page-break-inside: avoid;" dd:btnfloatingstyle="top-right">
                ...
                <xsl:if test="n:category-code"> 
                    <xsl:call-template name="getCodeDisplayAlphabetically">
                        <xsl:with-param name="node" select="n:category-code"/>
                        <xsl:with-param name="multipleValueSeparator" select="$multipleValueSeparator"/>
                    </xsl:call-template>
                </xsl:if>
                <xsl:call-template name="emitTableCellEnd"/>
                ...
                <xsl:if test="n:type-code">
                    <xsl:call-template name="getCodeDisplayAlphabetically">
                        <xsl:with-param name="node" select="n:type-code"/>
                        <xsl:with-param name="multipleValueSeparator" select="$multipleValueSeparator"/>
                    </xsl:call-template>
                </xsl:if>
                <xsl:call-template name="emitTableCellEnd"/>
