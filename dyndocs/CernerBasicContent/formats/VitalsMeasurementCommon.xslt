...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="xsl xs fn n cdocfx doc java-string xr-date-formatter">
	...
	<xsl:variable name="MeasurementDisplayPlusConversion" as="xs:string">
		<xsl:value-of select="'%s %s (%s %s)'"/>
	</xsl:variable>
	<xsl:variable name="MeasurementDisplay" as="xs:string">
	...
	<!-- displayImperialMeasurements will be overwritten by the locale specific format.If true, both imperial measurements for units of kg, cm, and degree celsius will display -->
	<xsl:variable name="displayImperialMeasurements" as="xs:boolean" select="false()"/>
	...
	<xsl:variable name="sMthdWeightEstimated" as="xs:string" select="'Estimated'"/>
	<xsl:variable name="sMthdWeightDosing" as="xs:string" select="'Dosing'"/>
	<xsl:variable name="sMthdWeightBirth" as="xs:string" select="'Birth'"/>
		...
		<measurement-concept type="Weight">
			...
			<concepts>
				<concept key="CERNER!ASYr9AEYvUr1YoRMCqIGfQ">
					<method><xsl:value-of select="$sMthdWeightBirth"/></method>
				</concept><!-- birth weight -->
				<concept key="CERNER!E9A8D345-C87A-4034-938A-BA2349967398">
					<method><xsl:value-of select="$sMthdEmpty"/></method>
				</concept> <!-- weight measured -->
				<concept key="CERNER!231D4897-58D8-4E6E-9B6A-C83764DE3E92">
					<method><xsl:value-of select="$sMthdWeightEstimated"/></method>
				</concept> <!-- weight estimated -->
				<concept key="CERNER!ABfQJgD4st77Y5Aqn4waeg">
					<method><xsl:value-of select="$sMthdWeightDosing"/></method>
				</concept> <!-- weight dosing -->
			</concepts>
		</measurement-concept>
	...
	<!-- Get temperature measurement value in DegC, CM, and kg from given value node. If the value is in DegC, CM, or kg display both Imperial and Metric results -->
	<!-- Parameters: -->
	<!-- 	measurement - display value of the vital/measurement -->
	<!-- 	unit - unit display of the vital/measurement -->
	<xsl:template name="tempOutputImperialMeasurements">
		<xsl:param name="interpretation"/>
		<xsl:param name="measurement"/>
		<xsl:param name="unit"/>
		
		<xsl:variable name="InterpretationResult">
			<xsl:choose>
				<xsl:when test="$interpretation">
					<xsl:variable name="Interpretation" as="xs:string">
						<xsl:variable name="NormalcyMean" as="xs:string">
							<xsl:value-of select="cdocfx:getCodeMeanByID($interpretation)"/>
						</xsl:variable>
						<xsl:choose>
		...
		<xsl:variable name="measurementWithUnit">
			<measurement>
				<xsl:choose>
					<xsl:when test="$displayImperialMeasurements">
						<xsl:choose>
							<xsl:when test="$unit='DEGC'">
                                <xsl:attribute name="displaySecondValue" select="true()"/>
								<xsl:attribute name="measurementValue" select="format-number((($measurement * 1.8) + 32), '#.0')"/>
								<xsl:attribute name="measurementUnit" select="$DegreeFahrenheit"/>
								<xsl:attribute name="measurementValueOriginal" select="$measurement"/>
								<xsl:attribute name="measurementUnitOriginal" select="$DegreeCelsius"/>
							</xsl:when>
							<xsl:when test="$unit='cm'">
                                <xsl:attribute name="displaySecondValue" select="true()"/>
								<xsl:attribute name="measurementValue" select="format-number(($measurement div 2.54), '#.00')"/>
								<xsl:attribute name="measurementUnit" select="'in'"/>
								<xsl:attribute name="measurementValueOriginal" select="$measurement"/>
								<xsl:attribute name="measurementUnitOriginal" select="'cm'"/>
							</xsl:when>
							<xsl:when test="$unit='kg'">
                                <xsl:attribute name="displaySecondValue" select="true()"/>
								<xsl:attribute name="measurementValue" select="format-number(($measurement * 2.205), '#.00')"/>
								<xsl:attribute name="measurementUnit" select="'lb'"/>
								<xsl:attribute name="measurementValueOriginal" select="$measurement"/>
								<xsl:attribute name="measurementUnitOriginal" select="'kg'"/>
							</xsl:when>						
							<xsl:otherwise>
                                <xsl:attribute name="displaySecondValue" select="false()"/>
								<xsl:attribute name="measurementValue" select="$measurement"/>
                                <xsl:attribute name="measurementValueOriginal" select="''"/>
								<xsl:choose>
									<xsl:when test="$unit='BPM'">
                                        <xsl:attribute name="displaySecondValue" select="false()"/>
                                        <xsl:attribute name="measurementUnit" select="'bpm'"/>
                                        <xsl:attribute name="measurementUnitOriginal" select="''"/>
                                    </xsl:when>
									<xsl:otherwise>
                                        <xsl:attribute name="displaySecondValue" select="false()"/>
                                        <xsl:attribute name="measurementUnit" select="$unit"/>
                                        <xsl:attribute name="measurementUnitOriginal" select="''"/>
                                    </xsl:otherwise>
								</xsl:choose>
							</xsl:otherwise>
					...
					<xsl:otherwise>
						...
						<xsl:choose>
							<xsl:when test="$unit='DEGC'">
                                <xsl:attribute name="measurementUnit" select="$DegreeCelsius"/>
                                <xsl:attribute name="measurementUnitOriginal" select="$DegreeCelsius"/>
                            </xsl:when>
							<xsl:when test="$unit='DEGF'">
                                <xsl:attribute name="measurementUnit" select="$DegreeFahrenheit"/>
                                <xsl:attribute name="measurementUnitOriginal" select="$DegreeCelsius"/>
                            </xsl:when>
							<xsl:when test="$unit='BPM'">
                                <xsl:attribute name="measurementUnit" select="'bpm'"/>
                                <xsl:attribute name="measurementUnitOriginal" select="'bpm'"/>
                            </xsl:when>
							<xsl:otherwise>
                                <xsl:attribute name="measurementUnit"  select="$unit"/>
                                <xsl:attribute name="measurementUnitOriginal" select="$unit"/>
                            </xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
		...
		<xsl:variable name="measurementValue" as="xs:string*" select="$measurementWithUnit/measurement/@measurementValue"/>
		<xsl:variable name="measurementValueOriginal" as="xs:string*" select="string($measurementWithUnit/measurement/@measurementValueOriginal)"/>
		<xsl:variable name="measurementUnit" as="xs:string*" select="$measurementWithUnit/measurement/@measurementUnit"/>
		<xsl:variable name="measurementUnitOriginal" as="xs:string*" select="$measurementWithUnit/measurement/@measurementUnitOriginal"/>
        <xsl:variable name="displaySecondValue" select="$measurementWithUnit/measurement/@displaySecondValue"/>
		
		<xsl:choose>
			<xsl:when test="$displaySecondValue = true()">
				<xsl:choose>
					<xsl:when test="$InterpretationResult = 'CRITICAL'">
						<xsl:copy-of select="$CriticalRangeImage"/>								
							<span style="padding-left: 2px; color:#CC0000;">
								<xsl:value-of disable-output-escaping="yes" select="java-string:format($MeasurementDisplayPlusConversion, ($measurementValue, $measurementUnit, $measurementValueOriginal, $measurementUnitOriginal))"/>
							</span>
					</xsl:when>
					<xsl:when test="$InterpretationResult = 'ABNORMAL'">
						<xsl:copy-of select="$AbnormalRangeImage"/>	
							<span style="padding-left: 2px; color:#814126;">
								<xsl:value-of disable-output-escaping="yes" select="java-string:format($MeasurementDisplayPlusConversion, ($measurementValue, $measurementUnit, $measurementValueOriginal, $measurementUnitOriginal))"/>
							</span>
					</xsl:when>
					<xsl:when test="$InterpretationResult = 'HIGH'">
						<xsl:copy-of select="$HighRangeImage"/>	
							<span style="padding-left: 2px; color:#FF6100;">
								<xsl:value-of disable-output-escaping="yes" select="java-string:format($MeasurementDisplayPlusConversion, ($measurementValue, $measurementUnit, $measurementValueOriginal, $measurementUnitOriginal))"/>
							</span>
					</xsl:when>
					<xsl:when test="$InterpretationResult = 'LOW'">
			...
			<xsl:otherwise>
				<xsl:choose>
					...
					<xsl:when test="fn:string-length($unit) and fn:not(fn:starts-with($unit, '%'))">
						<xsl:choose>
							<xsl:when test="$InterpretationResult = 'CRITICAL'">
								<xsl:copy-of select="$CriticalRangeImage"/>								
									<span style="padding-left: 2px; color:#CC0000;">
										<xsl:value-of disable-output-escaping="yes" select="java-string:format($MeasurementDisplay, ($measurementValue, $measurementUnit))"/>
									</span>
							</xsl:when>
							<xsl:when test="$InterpretationResult = 'ABNORMAL'">
								<xsl:copy-of select="$AbnormalRangeImage"/>	
					...
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="$InterpretationResult = 'CRITICAL'">
								<xsl:copy-of select="$CriticalRangeImage"/>								
									<span style="padding-left: 2px; color:#CC0000;">
										<xsl:value-of disable-output-escaping="yes" select="fn:concat($measurementValue,$measurementUnit)"/>
									</span>
							</xsl:when>
							<xsl:when test="$InterpretationResult = 'ABNORMAL'">
								<xsl:copy-of select="$AbnormalRangeImage"/>	
	...
	<xsl:template name="tempMeasurementDisplay">
		...
		<span dd:contenttype="PATCARE_MEAS">
			...
			<!-- Display results in Color with Indicator for Critical, Abnormal, High, and Low -->
			<xsl:choose>
				<xsl:when test="$interpretation">
					<xsl:variable name="NormalcyMean" as="xs:string">
						<xsl:value-of select="cdocfx:getCodeMeanByID($interpretation)"/>
	...
	<xsl:template name="tempMeasurementDisplayPtFacing">
		...
		<span dd:contenttype="PATCARE_MEAS" class="ddemrcontentitem">
			...
			<span>
				<xsl:variable name="ptMeasurement" select="cdocfx:getMeasurementValueDisplay($value, $DATE_SEQUENCE)"/>
				<xsl:call-template name="tempOutputImperialMeasurements">
					<xsl:with-param name="interpretation" select="$interpretation"/>
					<xsl:with-param name="measurement" select="$ptMeasurement"/>
	...
	<xsl:template name="handleMeasurements">
		...
		<!-- Separate the birth-weight measurement from all other measurement based on the concept-cki -->
		<!-- birthWeightMeasurement variable only contains birth-weight measurement -->
		<xsl:variable name="birthWeightMeasurement" select="$LatestMeasurement/*[n:event-type/@concept-cki = $birthWeightCKI]"/>
		
		<!-- LatestMeasurementWithoutBirthWeight variable contains all the measurement except birth-weight -->
		<xsl:variable name="LatestMeasurementWithoutBirthWeight" select="$LatestMeasurement/*[n:event-type/@concept-cki != $birthWeightCKI]"/>

		<xsl:for-each select ="$LatestMeasurementWithoutBirthWeight[n:event-end-dt-tm = $LatestMeasurementWithoutBirthWeight[1]/n:event-end-dt-tm]">
					...
					<xsl:choose>
						<xsl:when test="n:event-type/@concept-cki != $birthWeightCKI">
							<xsl:call-template name="tempMeasurementDisplay">
								...
								<xsl:with-param name="interpretation" select="if(fn:exists(@interpretation-code)) then @interpretation-code else $empty_string"/>
								<xsl:with-param name="unit" select="$unit"/>
		...
		<!-- Since the birth-weight measurement is sorted in descending order, Hence the first measurement will be the latest -->
		<!-- measurement. So which ever measurement is equal to its event-end-dt-tm pick up only those for display -->
		<xsl:for-each select="$birthWeightMeasurement[n:event-end-dt-tm = $birthWeightMeasurement[1]/n:event-end-dt-tm]">
			<xsl:variable name="cki" select="n:event-type/@concept-cki"/>
			<xsl:variable name="conceptMap" select="$measureConcept/concepts/concept[@key=$cki]"/>
			<xsl:variable name="unit">
				<xsl:if test="fn:exists(n:value/n:quantity/@unit-code)">
			...
			<xsl:choose>
				<xsl:when test="$isPatientFacingTemplate = false()">
					<xsl:if test="cdocfx:displayBirthWeight($birthDate,./n:event-end-dt-tm) = true()">
						<xsl:call-template name="tempMeasurementDisplay">
							<xsl:with-param name="abbreviation" select="if(fn:string-length($abbreviation)>0) then $abbreviation else $measureConcept/abbreviation"/>
							<xsl:with-param name="eventId" select="@event-id"/>
							<xsl:with-param name="value" select="n:value"/>
	...
	<xsl:template name="displayBPs">
		...
		<xsl:for-each select="$measureConcept/concepts/concept">
			...
			<xsl:if test="($bIncludeOrthoVitals = false()) and ($Method != $sMthdsitting and $Method != $sMthdstanding and $Method != $sMthdsupine)">
				...
				<xsl:if test="fn:exists($matchingDBP)">
					<span class="ddgrouper ddremovable">
						...
						<xsl:call-template name="tempMeasurementDisplay">
							...
							<xsl:with-param name="interpretation" select="if(fn:exists($SystolicBPMeasurement[1]/@interpretation-code)) then $SystolicBPMeasurement[1]/@interpretation-code else $empty_string"/>
						...
						<xsl:call-template name="tempMeasurementDisplay">
							...
							<xsl:with-param name="interpretation" select="if(fn:exists($matchingDBP[1]/@interpretation-code)) then $matchingDBP[1]/@interpretation-code else $empty_string"/>
							<xsl:with-param name="unit" select="$empty_string"/>
							<xsl:with-param name="method" select="$empty_string"/>
							<xsl:with-param name="removable" select="fn:false()"/>
							<xsl:with-param name="appendSpace" select="fn:false()"/>
						...
						<xsl:if test="fn:exists($HRMeasurement)">
							<span class="ddgrouper ddremovable">
								<xsl:call-template name="tempMeasurementDisplay">
									...
									<xsl:with-param name="interpretation" select="if(fn:exists($HRMeasurement[1]/@interpretation-code)) then $HRMeasurement[1]/@interpretation-code else $empty_string"/>
									<xsl:with-param name="unit" select="$empty_string"/>
									<xsl:with-param name="method" select="$empty_string"/>
									<xsl:with-param name="removable" select="fn:false()"/>
									<xsl:with-param name="appendSpace" select="fn:false()"/>
