...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    exclude-result-prefixes="xsl xs fn xr-date-formatter n cdocfx doc dd java-string">
                        ...
                        <xsl:for-each select="n:report/n:clinical-data/n:follow-up-data/n:follow-up">
                            ...
                            <tr class="ddemrcontentitem ddremovable" dd:btnfloatingstyle="top-right">
                                ...
                                <td style="vertical-align: top; padding-left: 6px;padding-bottom:15px; word-wrap: break-word;">
                                    <!--Adding "FollowupWith" label only when we have provider/location details-->
                                    <xsl:if test="fn:exists(./@provider-name) and (fn:string-length(./@provider-name)) &gt; 0">
                                ...
                                <td style="vertical-align: top; padding-left: 6px;padding-bottom: 15px; word-wrap: break-word;">
                                    <table>
                                        <tr>
                                            <!--Adding "Why" label only when comment sections are not empty-->
                                            <xsl:if test="n:comment[(fn:string-length(.)) &gt; 0]">
                                        ...
                                        <tr>
                                            <xsl:variable name="address" select="n:follow-up-location/n:follow-up-address"/>
                                            <!--Adding "Where" label only when the address fields are having data-->
                                            <xsl:if test="(n:follow-up-location/@follow-up-phone-freetext[(fn:string-length(.)) &gt; 0]) or ($address/@address1[(string-length(.)) &gt; 0]) or ($address/@address2[(string-length(.)) &gt; 0]) or ($address/@address3[(string-length(.)) &gt; 0]) or ($address/@address4[(string-length(.)) &gt; 0]) or ($address/@city[(string-length(.)) &gt; 0]) or ($address/@state[(string-length(.)) &gt; 0]) or ($address/@zip[(string-length(.)) &gt; 0])">
                                                <td style="text-align:right; vertical-align: top; white-space: nowrap; font-weight: bold;">
                                                    <xsl:value-of select="$Where"/>
                                                </td>
                                            </xsl:if>
    ...
    <xsl:template name="DisplayWhen">
        ...
        <!--Adding "When" label only when we have follow-up-unit or follow-up-dt-tm or follow-up-within-range or if-needed 'true' so that "When" label will not come without having any data-->
        <xsl:if test="($FollowupWhen/@follow-up-unit != '' and $FollowupWhen/@follow-up-unit != 'IGNORE') or (fn:exists($FollowupWhen/n:follow-up-dt-tm) and (fn:string-length($FollowupWhen/n:follow-up-dt-tm)&gt; 0)) or ($FollowupWhen/@follow-up-within-range!='') or ($FollowupWhen/@if-needed = 'true')">
            <span style="font-weight: bold;">
                <xsl:value-of select="$When"/>
            </span>
            <xsl:text disable-output-escaping="yes"><![CDATA[&#160;]]></xsl:text>
