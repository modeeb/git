...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="Given" as="xs:string">
		<xsl:value-of select="'Administrerat'"/>
	</xsl:variable>
	
	<xsl:variable name="NotGiven" as="xs:string">
		<xsl:value-of select="'Ej administrerat'"/>
	</xsl:variable>
	
	<xsl:variable name="Recorded" as="xs:string">
		<xsl:value-of select="'Registrerat'"/>
	</xsl:variable>
	...
	<xsl:variable name="DayPrecisionFormatter" as="xs:string">
		<xsl:value-of select="'%3$s-%1$s-%2$s'"/> <!-- %1$s is month, %2$s is day, %3$s is year -->
	</xsl:variable>
