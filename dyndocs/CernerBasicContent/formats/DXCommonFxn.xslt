...
<xsl:stylesheet version="2.0" 
	...
	<!-- If set to true, ExcludeRFVDiagnosisType will not display any Diagnosis with a type of Reason for Visit (RFV) -->
	<xsl:variable name="ExcludeRFVDiagnosisType" as="xs:boolean" select="true()"/>
	...
	<xsl:function name="cdocfx:getDisplayableDiagnoses">
		...
		<xsl:variable name="dxList" as="element()*">
			<xsl:choose>
				<xsl:when test="$ExcludeRFVDiagnosisType">
					<xsl:choose>
						...
						<xsl:when test="count($clinicalDiagnosis[@clinical-priority = 0 and @is-active='true' and not(cdocfx:getCodeMeanByID(@type-code)='RFV')]) = count($clinicalDiagnosis[@is-active='true' and not(cdocfx:getCodeMeanByID(@type-code)='RFV')])">
							<xsl:perform-sort select="$clinicalDiagnosis[@is-active='true' and not(cdocfx:getCodeMeanByID(@type-code)='RFV')]">
						...
						<xsl:otherwise>
							<!-- if any one of the diagnosis is prioritized, then display only prioritized diagnosis -->
								<xsl:perform-sort select="$clinicalDiagnosis[@is-active='true' and cdocfx:shouldDisplayDiagnosis(.,$entityIdList)=true() and not(cdocfx:getCodeMeanByID(@type-code)='RFV')]">
	...
	<!-- getOrder function which creates an element list of orders which match with the nomenID/id -->
	<!-- Parameters -->
	<!-- dxNomenId : String nomenclature Id -->
	<!-- dxId : String Id -->
	<!-- Orders : array of order -->
	<!-- returns : a newly created element orders list for matching nomenID/id -->
	<xsl:function name="cdocfx:getOrder">
		...
		<xsl:variable name="Order" as="element()*">
			<xsl:for-each select="$Orders">
				<xsl:choose>
					<xsl:when test="./n:diagnosis/@group-id &gt; 0 and $dxId = ./n:diagnosis/@group-id">
						<xsl:copy-of select="."/>
					</xsl:when>
					<xsl:when test="./n:diagnosis/@group-id = 0 and $dxNomenId = ./n:diagnosis/@nomenclature-id">
						<xsl:copy-of select="."/>
					</xsl:when>
				</xsl:choose>
			</xsl:for-each>
	...
	<!-- getDiagnosesConfirmationDisplay function which returns confirmation status of given diagnosis -->
	<!-- Parameters -->	
	<!--    dx - diagnosis node-->	
	<!--    returns: confirmation status string enclosed in parenthesis. Blank if confirmation code is not charted or confirmation status is Confirmed.-->
	<xsl:function name="cdocfx:getDiagnosesConfirmationDisplay" as="xs:string">
		<xsl:param name="dx"/>		
		<xsl:choose>
			<xsl:when test="$dx/@confirmation-code and not(cdocfx:getCodeMeanByID($dx/@confirmation-code)='CONFIRMED')">
				<xsl:choose>
					<xsl:when test="cdocfx:getCodeDisplayByID($dx/@confirmation-code)!=''">
						<xsl:value-of select="concat(fn:string(' '),'(',cdocfx:getCodeDisplayByID($dx/@confirmation-code),')')"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="''"/>
					</xsl:otherwise>
